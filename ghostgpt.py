...
def set_max_tokens():
    global max_tokens
    max_tokens = int(max_tokens_spinbox.get())
...
def send_message():
    ...
    chat_log.tag_add("user", chat_log.index(
        "end - 2 lines linestart"), chat_log.index("end - 2 lines lineend"))
    chat_log.see("end")
    entry.delete(0, "end")
    response = talk_to_gpt(message)
    chat_log.insert("end", f"GhostGPT: {response}\n")
    chat_log.tag_configure("gpt", foreground="cyan")
    chat_log.tag_add("gpt", chat_log.index(
        "end - 2 lines linestart"), chat_log.index("end - 2 lines lineend"))
    chat_log.see("end")
...
chat_log = ScrolledText(root, wrap="word", width=50,
                        height=20, bg="black", fg="white", state="disabled")
chat_log.grid(row=0, column=0, columnspan=3, padx=5, pady=5)

send_button = tk.Button(
    root, text="Send", command=send_message, bg="black", fg="white", width=10)
send_button.grid(row=1, column=1, padx=5, pady=5)

clear_button = tk.Button(
    root, text="Clear", command=clear_chat, bg="black", fg="white", width=10)
clear_button.grid(row=2, column=1, padx=5, pady=5)

api_key_button = tk.Button(
    root, text="API Key", command=create_and_open_api_key_file, bg="black", fg="white", width=10)
api_key_button.grid(row=3, column=1, padx=5, pady=5)

# Add max tokens Spinbox and label
max_tokens_label = tk.Label(root, text="Max tokens:", bg="black", fg="white")
max_tokens_label.grid(row=4, column=1, padx=5, pady=5, sticky="w")
max_tokens_spinbox = Spinbox(
    root, from_=1, to=2048, width=5, command=set_max_tokens)
max_tokens_spinbox.grid(row=4, column=1, padx=5, pady=5, sticky="e")
max_tokens = 100  # Set initial max tokens value
max_tokens_spinbox.delete(0, 'end')
max_tokens_spinbox.insert(0, max_tokens)


entry = tk.Entry(root, width=40, bg="black", fg="white")
entry.grid(row=1, column=0, padx=5, pady=5)

export_button = tk.Button(root, text="Export Chat",
                          command=export_chat, bg="black", fg="white", width=10)
...
start_button = tk.Button(root, text="Start Talking",
                         command=listen_to_user, bg="black", fg="white", width=10)
...
version_label = tk.Label(root, text="GhostGPT by Ghost v.1.0.0",
                         fg="red", bg="black", font=("Helvetica", 13, "italic"))
version_label.grid(row=5, column=0, padx=5, pady=0)

github_link = tk.Label(root, text="github.com/ghostkiwicoder", fg="cyan",
                       cursor="hand2", bg="black", font=("Helvetica", 11, "italic"))
